create table tasks (
  id bigserial primary key,
  project_id bigint not null references projects(id),
  dataset_id bigint references datasets(id),
  snapshot_id bigint references dataset_snapshots(id),
  title text not null,
  description text not null default '',
  assignee text not null default '',
  status text not null,
  priority text not null,
  due_at timestamptz,
  last_activity_at timestamptz not null default now(),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint tasks_status_check check (status in (
    'ready','in_progress','submitted','reviewing','rework_required','accepted','published','closed'
  )),
  constraint tasks_priority_check check (priority in ('low','normal','high','critical'))
);

create index idx_tasks_project on tasks(project_id);
create index idx_tasks_project_status on tasks(project_id, status);
create index idx_tasks_project_last_activity on tasks(project_id, last_activity_at);
